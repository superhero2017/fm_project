...
The first section of this documentation covers Feedme specific issues. The remaining text is part of `create-react-app`.

## Table of Contents

- [Running the Feedme Frontend](#running-the-feedme-frontend)
...
## Running the Feedme frontend

In order to run the frontend locally for development purposes, clone the repo and follow these steps:

Create a `.env` file in the project root defining these variables:

```
# Only vars starting with `REACT_APP` are imported

PORT = 8020

REACT_APP_API_URL = http://localhost:8010
REACT_APP_STRIPE_KEY = pk_test_pgtKRb0bhamHi1Vxu3WMhTl1
```

This will tell webpack which servers and ports to point to.
...
You can now access the frontend from `http://localhost:8020`. However, the backend will not be running so functionality will be limited. The backend must be setup to run at the url provided in the `.env` file

Addendum: I recommend adding an entry to `/etc/hosts` that says `127.0.0.1 feedmedev.io`. Then you can access the frontend from `http://feedmedev.io:8020`. If you use a mac, make sure to run Run sudo pkill mDNSResponder to flush DNS cache after modifying `/etc/hosts`.
